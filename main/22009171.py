...
salary_per_town_categories_merge = salary_per_town_categories.merge(population,
                                                                    left_on = 'CODGEO',
                                                                    right_on = 'CODGEO',
                                                                    how = 'inner',
...
male = male[['CODGEO', 'LIBGEO_left',
             'SNHME14', 'SNHMO14',
             'SNHM1814', 'SNHM2614', 'SNHMH5014'
             ]]
male = male.rename(columns = {'CODGEO' : 'CODGEO',
                              'LIBGEO_left' : 'LIBGEO',
                              'SNHME14': 'net_salary_per_hour_employee',
                              'SNHMO14': 'net_salary_per_hour_worker',
                              'SNHM1814' : 'net_salary_18_to_25',
                              'SNHM2614' : 'net_salary_26_to_50' ,
                              'SNHMH5014' : 'net_salary_more_than_50yrs'
                              }
                   )
male = male.drop_duplicates()
male_top_10 = male.sort_values(['net_salary_per_hour_employee',
                                'net_salary_per_hour_worker'
                                ],
                               ascending = False).head(10)

# filtering female data
female = salary_per_town_categories_merge[salary_per_town_categories_merge['SEXE'] == 2]
female = female[['CODGEO', 'LIBGEO_left',
                 'SNHMFE14', 'SNHMFO14',
                 'SNHMF1814', 'SNHMH2614', 'SNHMF5014'
                 ]]
female = female.rename(columns = {'CODGEO' : 'CODGEO',
                                  'LIBGEO_left' : 'LIBGEO',
                                  'SNHMFE14' : 'net_salary_per_hour_employee',
                                  'SNHMFO14' : 'net_salary_per_hour_worker',
                                  'SNHMF1814' : 'net_salary_18_to_25',
                                  'SNHMH2614' : 'net_salary_26_to_50',
                                  'SNHMF5014' : 'net_salary_more_than_50yrs'
                                  }
                       )
female = female.drop_duplicates()
female_top_10 = female.sort_values(['net_salary_per_hour_employee', 'net_salary_per_hour_worker'],
                                   ascending = False).head(10)
female_top_10 = female_top_10[["LIBGEO", "net_salary_18_to_25",
                               "net_salary_26_to_50",
                               "net_salary_more_than_50yrs"
                               ]]

# filtering expensive cities' data
...
# filtering less expensive cities' data
...
# Getting the distance coulmn from source data
distance = name_geographic_information[['code_insee','éloignement']]
dtype = {'CODGEO' : int}
expensive_cities=expensive_cities.astype(dtype)
# Merging the distance column with the other oder DFs on CODGEO columns
distance_pd_merged = pd.merge(expensive_cities,
                              distance,
                              how = 'left',
                              left_on = 'CODGEO',
                              right_on = 'code_insee')
...
# filtering the columns needed for plotting the expensive cities
expensive_distance_pd_merged = pd.merge(expensive_cities,
                                        distance,
                                        how = 'left',
                                        left_on = 'CODGEO',
                                        right_on = 'code_insee')
expensive_distance_pd_merged[['LIBGEO',
                              'SNHM14',
                              'éloignement'
                              ]].sort_values('SNHM14', ascending = False)
...
# filtering the columns needed for plotting the less expensive cities
pas_cher_distance_pd_merged = pd.merge(pas_cher_cities_3,
                                       distance,
                                       how='left',
                                       left_on='CODGEO',
                                       right_on='code_insee'
...
salary_data = salary_per_town_categories.sort_values(by = ['percentage_wage_gap'],
                                                     ascending = False
                                                     ).head(15)
...
plot_1 = ax3.plot(expensive_cities_2['LIBGEO'],
                  expensive_cities_2['SNHM14'],
                  label = "Net Mean Salary"
                  )
...
plot_2 = ax31.plot(expensive_distance_pd_merged['LIBGEO'],
                   expensive_distance_pd_merged['éloignement'],
                   label = "Distance from Paris", color = 'blue'
                   )
...
base_establissement_pre_tranche = base_establissement_pre_tranche[['LIBGEO', 'E14TS1',
                                                                   'E14TS6', 'E14TS10',
                                                                   'E14TS20', 'E14TS50',
...
paris["employees betweeen 1 to 20"] = paris["More than 20 employees"] \
                                      + paris["More than 10 employees"] \
                                      + paris["More than 6 employees"] \
                                      + paris["More than 1 employees"]
paris = paris.drop(columns = ["More than 20 employees",
...
plt.legend(labels=["More than 500 employees",
           ...
           bbox_to_anchor = (1.05, 0.75), fontsize = 10)
